...
/**
 * Clase que encárgase de mostrar as opcións do menú principal. Terá os
 * atributos: "postsShowed", que terá como valor inicial 10 e que indica o
 * número de publicacións que se mostran na visualización (este atributo será o
 * único que terá un método "get()" público) e "profileController" que mantén a
...
public class TextProfileView implements ProfileView {
     ...
     *
     * @param ownProfile True si ve su propio perfil, false si ve el de otro.
     * @param scanner Variable para pedir por pantalla.
     * @param profile Perfil del usuario.
     */
    private void changeStatus(boolean ownProfile, Scanner scanner, Profile profile) {
        if (ownProfile) {
            System.out.println("¿Cuál es tu nuevo estado?");
            String status = scanner.nextLine();
            profileController.updateProfileStatus(status);
        } else {
            System.out.println("Sólo puedes cambiar el estado en tu propia biografía.");
    ...
    public void showProfileMenu(Profile profile) {
        ...
        switch (choice) {
            ...
            case 3:
                ...
                break;
            case 4:
                showBiography(ownProfile, scan, profile);
                break;
            case 5:
    ...
    private void writeNewPost(Scanner scanner, Profile profile) {
        System.out.println("Escribe el texto de la publicación: ");
    ...
    private void commentPost(Scanner scanner, Profile profile) {
        int postNumber = selectElement("Introduce el número de la publicación que quieres comentar:", profile.getPosts().size(), scanner);
    ...
    private void addLike(Scanner scanner, Profile profile) {
        int postNumber = selectElement("Introduce el número de la publicación:", profile.getPosts().size(), scanner);
    ...
    private void showBiography(boolean ownProfile, Scanner scanner, Profile profile) {
        if (ownProfile) {
            int friendToShow = selectElement("Elige al amigo que quieres ver: (NÚMERO)", profile.getFriends().size(), scanner);
    ...
    private void proccessFriendshipRequest(boolean ownProfile, Scanner scanner, Profile profile, boolean accept) {
        if (ownProfile) {
            if (accept) {
                // Si quiere aceptar la solicitud de amistad
        ...
        if (ownProfile) {
            ...
            switch (option) {
                case 1:
                    ...
                    // Si lo quiere responder, lo marcamos como leidos
    ...
    private void showOldPosts(Scanner scanner, Profile profile) {
        do {
            System.out.println("¿Cúantas publicaciones quieres visualizar?");
